...
def telemetry_to_gcs_thread():
    ...
    try:
        ...
        print(f"[SPECK Drone] UDP bind failed on {DRONE_HOST}:{PORT_DRONE_LISTEN_PLAINTEXT_TLM} -> {e}; using 0.0.0.0")
        sock.bind(("0.0.0.0", PORT_DRONE_LISTEN_PLAINTEXT_TLM))
...
def commands_from_gcs_thread():
    ...
    try:
        ...
        print(f"[SPECK Drone] UDP bind failed on {DRONE_HOST}:{PORT_DRONE_LISTEN_ENCRYPTED_CMD} -> {e}; using 0.0.0.0")
        sock.bind(("0.0.0.0", PORT_DRONE_LISTEN_ENCRYPTED_CMD))
