...
def perform_key_exchange():
    ...
    while retry_count < max_retries:
        try:
            ...
            retry_count += 1
            print(f"[KYBER-1024 Drone] Connection attempt {retry_count}/{max_retries} failed: {e}")
    ...
    raise RuntimeError(f"[KYBER-1024 Drone] Could not connect to GCS after {max_retries} attempts")
...
def telemetry_to_gcs_thread():
    ...
    try:
        ...
        print(f"[KYBER-1024 Drone] UDP bind failed on {DRONE_HOST}:{PORT_DRONE_LISTEN_PLAINTEXT_TLM} -> {e}; using 0.0.0.0")
    ...
    print(f"[KYBER-1024 Drone] Forwarding encrypted TLM to {GCS_HOST}:{PORT_GCS_LISTEN_ENCRYPTED_TLM}")
    ...
    while True:
        try:
            ...
            print(f"[KYBER-1024 Drone] Telemetry thread error: {e}")
            time.sleep(0.1)
...
def commands_from_gcs_thread():
    ...
    try:
        ...
        print(f"[KYBER-1024 Drone] UDP bind failed on {DRONE_HOST}:{PORT_DRONE_LISTEN_ENCRYPTED_CMD} -> {e}; using 0.0.0.0")
    ...
    print(f"[KYBER-1024 Drone] Forwarding decrypted CMD to {DRONE_HOST}:{PORT_DRONE_FORWARD_DECRYPTED_CMD}")
    ...
    while True:
        try:
            ...
            print(f"[KYBER-1024 Drone] Command thread error: {e}")
            time.sleep(0.1)
