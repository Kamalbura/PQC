...
def handle_key_exchange():
    ...
    try:
        ...
        print(f"[KYBER-512 GCS] TCP bind failed on {GCS_HOST}:{PORT_KEY_EXCHANGE} -> {e}; using 0.0.0.0")
...
def drone_to_gcs_thread():
    ...
    try:
        ...
        print(f"[KYBER-512 GCS] UDP bind failed on {GCS_HOST}:{PORT_GCS_LISTEN_ENCRYPTED_TLM} -> {e}; using 0.0.0.0")
    ...
    print(f"[KYBER-512 GCS] Forwarding decrypted TLM to {GCS_HOST}:{PORT_GCS_FORWARD_DECRYPTED_TLM}")
    ...
    while True:
        try:
            ...
            print(f"[KYBER-512 GCS] Telemetry thread error: {e}")
            time.sleep(0.1)
...
def gcs_to_drone_thread():
    ...
    try:
        ...
        print(f"[KYBER-512 GCS] UDP bind failed on {GCS_HOST}:{PORT_GCS_LISTEN_PLAINTEXT_CMD} -> {e}; using 0.0.0.0")
    ...
    print(f"[KYBER-512 GCS] Forwarding encrypted CMD to {DRONE_HOST}:{PORT_DRONE_LISTEN_ENCRYPTED_CMD}")
    ...
    while True:
        try:
            ...
            print(f"[KYBER-512 GCS] Command thread error: {e}")
            time.sleep(0.1)
